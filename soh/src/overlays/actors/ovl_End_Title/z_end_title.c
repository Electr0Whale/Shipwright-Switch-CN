...
void EndTitle_DrawFull(Actor* thisx, PlayState* play) {
    ...
    // SOH [Chinese] - use the Simplified Chinese "The End" / "The Legend of Zelda" /
    // "Ocarina of Time" tiles when Chinese is selected.
    void* theEndTex = (gSaveContext.language == LANGUAGE_CHI) ? sTheEndCHITex : sTheEndTex;
    void* tlozTex = (gSaveContext.language == LANGUAGE_CHI) ? sTheLegendOfZeldaCHITex : sTheLegendOfZeldaTex;
    void* ootTex = (gSaveContext.language == LANGUAGE_CHI) ? sOcarinaOfTimeCHITex : sOcarinaOfTimeTex;
    ...
    gDPLoadTextureTile(OVERLAY_DISP++, tlozTex, G_IM_FMT_IA, G_IM_SIZ_8b, 120, 24, 0, 0, 120 - 1, 24 - 1, 0,
                       G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 0, 0, 0, 0);
    ...
    gDPLoadTextureTile(OVERLAY_DISP++, ootTex, G_IM_FMT_IA, G_IM_SIZ_8b, 112, 16, 0,
                       0, 112 - 1, 16 - 1, 0,
                       G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 0, 0, 0, 0);
